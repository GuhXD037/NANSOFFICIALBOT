...
let handler = async (m, { conn, usedPrefix, command }) => {
	...
	if (!global.db.data.chats[m.chat].nsfw && m.isGroup) throw `❗Grup tidak mendukung konten nsfw \n\n Untuk mengaktifkan tipe \n*${usedPrefix}enable* nsfw`
   var {age} = global.db.data.users[m.sender]
   if (age <17) throw m.reply(`❎ Anda masih di bawah umur! Kembalilah saat Anda berusia di atas 18 tahun`) 
